    ...
    def __init__(self, config: Dict, seed: int = 42):
        ...
        for agent_id in config["env_para"]["agent_ids"]:
            if AgentType.PREDATOR in agent_id:
                self._predators.append(agent_id)
            elif AgentType.PREY in agent_id:
                self._preys.append(agent_id)
            else:
                raise ValueError(
                ...
                agents_alive=smarts_agent_interface.AgentsAliveDoneCriteria(
                    agent_lists_alive=[
                        smarts_agent_interface.AgentsListAlive(
                            agents_list=self._preys, minimum_agents_alive_in_list=1
                        ),
                    ]
                ),
            ),
        ...
        prey_interface = smarts_agent_interface.AgentInterface(
            ...
            done_criteria=smarts_agent_interface.DoneCriteria(
                ...
                agents_alive=smarts_agent_interface.AgentsAliveDoneCriteria(
                    agent_lists_alive=[
                        smarts_agent_interface.AgentsListAlive(
                            agents_list=self._predators, minimum_agents_alive_in_list=1
                        ),
                    ]
                ),
            ),
        ...
        self._action_space = array_spec.BoundedArraySpec(shape=(), dtype=np.int32, minimum=0, maximum=config["model_para"]["action_dim"], name="action") 
        # Observation space
        self._observation_space = array_spec.BoundedArraySpec(shape=(config["model_para"]["observation_dim1"]), dtype=np.uint8, minimum=0, maximum=255, name="observation")
