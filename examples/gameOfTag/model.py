    ...
    def __init__(
        ...
        with tf.variable_scope(scope_name):
            ...
            self.action_logstd = tf.Variable(
                np.full((num_actions), np.log(0.4), dtype=np.float32),
                name="action_logstd",
            )
            ...
            if clip_action_space:
                ...
                action_min = tf.reshape(
                    tf.tile(
                        tf.convert_to_tensor(action_min, dtype=tf.float32), (num_envs,)
                    ),
                    (num_envs, num_actions),
                )
                action_max = tf.reshape(
                    tf.tile(
                        tf.convert_to_tensor(action_max, dtype=tf.float32), (num_envs,)
                    ),
                    (num_envs, num_actions),
                )
