...
class PolicyGraph():
    ...
    def __init__(self, input_states, taken_actions,
        ...
        with tf.variable_scope(scope_name):
            # Construct model
            self.conv1 = tf.layers.conv2d(input_states, filters=8, kernel_size=12, strides=4, 
                activation=tf.nn.leaky_relu, padding="valid", name="conv1")
            self.conv2 = tf.layers.conv2d(self.conv1, filters=16, kernel_size=6, strides=4, 
                activation=tf.nn.leaky_relu, padding="valid", name="conv2")
            self.conv3 = tf.layers.conv2d(self.conv2, filters=32, kernel_size=3, strides=2, 
                activation=tf.nn.leaky_relu, padding="valid", name="conv3")
            self.flatten = tf.layers.flatten(self.conv3, name="flatten")
            self.shared_features = tf.layers.dense(self.flatten, 256, 
